...
First make sure zephyr is installed as service.

```bash

cd cryptonote-zephyr-pool
mv zephyr /usr/bin
cp ombre.service /lib/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable ombre
```
Add user so that zephyr will run under a user not root

```bash
sudo useradd -m zephyrdaemon -d /home/zephyrdaemon


Make sure that the zephyr and ombre-wallet-rpc daemons are running:
...
When both are running it's worth to make sure the zephyr has it's blockchain
synced. If this is not the case your pool will show errors with "Core is busy"
until zephyr is synchronized.
